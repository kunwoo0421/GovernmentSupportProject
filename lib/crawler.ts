    ...
    try {
        ...
        try {
            ...
            if (headerCode && headerCode !== '00' && headerCode !== 'NORMAL_SERVICE') {
                console.log(`[OpenAPI] MSS API Error Code: ${headerCode}`);
                // Don't return empty immediately, some APIs return code in body but still have data? 
                // Usually not. But let's log and return empty.
                return [];
            }
            ...
            console.log(`[OpenAPI] MSS fetched ${notices.length} items.`);
            return notices;
        } catch (e) {
